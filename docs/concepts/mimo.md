# Mamba-3-Lite — MIMO Head Mixing: the fully-connected cross-head mixer
...
3. **The mixer sees only `Y.real` — no complex mixing.** `models/ssd_complex.py:ssd_complex_chunkwise` returns `Y_diag + Y_off` truncated to its real part, so `MIMO` receives float32 real activations and `W` is real. The complex state is an internal representation of the recurrence ([03-complex-ssd](../concepts/ssd-theory.md)); the model observes only its real projection, and the mixer mixes what the model observes. Mixing the complex output instead would double the width (and cost), require a complex $W$, and — because `.real` discards the imaginary part before the residual add — would train the imaginary half of $W$ against zero gradient.
...
All three run on CPU and are part of the 28 passing tests on a CPU box (the 5 GPU-gated tests are the Triton/e2e paths). For the API contract (signature, shapes, the flag) see [Mamba-3-Lite — Model Reference](../references/model-reference.md); the surrounding block wiring is in [Mamba-3-Lite — Model Reference](../references/model-reference.md) and [Mamba-3-Lite — Model Reference](../references/model-reference.md); the init scheme as a whole (including the `_identity_init` escape hatch) is summarized in [Mamba-3-Lite — Block Anatomy and Numerical Stability](../concepts/block-and-stability.md).

---

## References

- [Mamba-3-Lite — SSD Theory](ssd-theory.md) — what the scan computes before the mixer (`Y.real` contract).
- [Mamba-3-Lite — Block Anatomy and Numerical Stability](block-and-stability.md) — the `_identity_init` escape hatch in `models/transformer.py:Mamba3Transformer._init_weights`.
- [Mamba-3-Lite — SSD Foundations](state-space-foundations.md) — the SISO recurrence the mixer extends.
- [Mamba-3-Lite — Model Reference](../references/model-reference.md) — `models/mimo.py:MIMO` API, the call site in `models/mamba_block.py:Mamba3Block._forward_impl`.
